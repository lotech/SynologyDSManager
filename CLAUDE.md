...
| `AppModel.swift` | `@Observable @MainActor` singleton (`AppModel.shared`). Owns the polling loop, task list, bandwidth, notifications, pending torrent paths, the menu-bar title, the Dock-icon finished-count badge, and the `SynologyAPI` instance. All screens read/write via `AppModel.shared`; replaced the mutable globals that used to live in `Shared.swift`. |
| `Shared.swift` | Utility functions only: `prettifyBytesCount`, `prettifySpeed`, `Double.round(to:)`. All mutable globals (`mainViewController`, `currentViewController`, etc.) were removed in Phase 4. |
...
| `ViewControllers/` | SwiftUI views, one per screen (`DownloadsView` — a sortable, multi-select `Table` with a right-click context menu — plus `SettingsView`, `AddDownloadView`, `BTSearchView`, `ChooseDestView`, `AboutView`), plus `DestinationPicker` (the SwiftUI download-destination pop-up that replaced the XIB-backed `DestinationView`). The "Other…" directory browser is presented via `ChooseDestHostingController` (`NSHostingController`), the one remaining AppKit hosting shim. |
